"""Tests for the model registry + support-metadata guard API (atf_dsp.models)."""
import atf_dsp
from atf_dsp import models


def test_model_info_hardware_confirmed_m54():
    """The M 5.4DSP is the one fully-supported, hardware-confirmed model."""
    info = atf_dsp.model_info(0x2008)  # by PID
    ...
    assert atf_dsp.model_info("MATCH M 5.4DSP") == info
...
def test_model_info_discoverable_but_unsupported():
    ...
    info = atf_dsp.model_info("MATCH UP 10DSP MK2")
...
def test_model_info_multirate_fs_from_at01_basename():
    ...
    info = atf_dsp.model_info("BRAX DSP")
...
def test_model_info_unknown_pid():
    info = atf_dsp.model_info(0x9999)
...
def test_model_info_flags_incomplete_topology():
    ...
    info = atf_dsp.model_info("HELIX DSP ULTRA")
